...
filter_ods <- function(max_dt, data) {
  ...
  data$opioid_related <- grepl(
    paste0(opioidlist, collapse = "|"),
    data$primarycause
  )
  ...
  data %>%
    ...
    reframe(
      ...
      age = as.numeric(age),
      sex = factor(
        ifelse(gender %in% c("Male", "Female"), gender, NA),
        levels = c("Male", "Female"),
        labels = c("Male", "Female")
      ),
      race = factor(
        ifelse(race %in% c("Other", "Unknown"), "Other", race),
